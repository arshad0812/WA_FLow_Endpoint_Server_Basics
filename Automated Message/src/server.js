...
app.post("/webhook", async (req, res) => {
  try {
    ...
    const fromNumber = messageObj.from; // Sender WhatsApp number
    const msgBody = messageObj.text?.body;

    console.log("Received message:", msgBody, "from:", fromNumber);

    // REPLY ONLY IF NUMBER MATCHES
    const specificNumber = "916381794798"; // ← your target WhatsApp number

    if (fromNumber === specificNumber) {
      await sendWhatsappMessage(fromNumber, "Hello! This is an automated reply.");
...
async function sendWhatsappMessage(to, message) {
  try {
    await axios.post(
      `https://graph.facebook.com/v18.0/771572809383723/messages`,
      {
      ...
      {
        headers: {
          Authorization: `Bearer EAAa3TZA46FzIBP4X9tizRqZBF6lCAFZAWJPUJl7mXjbhXE2bPO0lPochcTfDPZCZAylMyeZBqZCXDkXVahQWAZCC0rjovWVYndGuXEguIxf4mtg65s8VDFJdixUp84z70ZAgMYgvNSTiNbG4QMlAw8RczTmwHoJp5qpg4cZBd8TNkZAALcDZAZBxxQCK6w4XsF8SpdaPCBRVYNEaWyWpB1ZAz3j6dCN19UFaVyAVaxicayXlPHflQi9AZDZD`,
          "Content-Type": "application/json",
        }
      }
    );
  } catch (error) {
    console.error("Sending error:", error.response?.data || error);
  }
}
